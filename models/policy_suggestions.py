...
def visualize_monthly_spending_trend(monthly_spending):
    if not monthly_spending.empty:
        ...
        # Simple Explanation
        st.write("""**What this graph shows:** This graph displays the total spending over time. The x-axis represents the months, 
            and the y-axis shows how much was spent in each month. The color gradient shows changes in spending, with cooler tones 
            representing lower spending and warmer tones showing higher spending. 
            **Key Takeaways:** - Look for trends in the graph: are expenses rising, falling, or staying constant? 
            - Peaks may indicate months of higher-than-normal expenses, which could be useful to understand and plan for future spending.""")
...
def visualize_spending_categories(monthly_spending):
    ...
    # Simple Explanation
    st.write("""**What this graph shows:** This graph breaks down your monthly expenses into different categories: Low, Medium, and High. 
            Each bar represents how many months fall into each category, indicating the frequency of that spending level. 
            **Key Takeaways:** - If most of your expenses fall into the 'Medium' category, this suggests that your spending is generally moderate. 
...
def visualize_roi_bar(policy_data):
    ...
    # Simple Explanation
    st.write("""**What this graph shows:** This bar chart displays the average expected ROI (Return on Investment) for each ROI category 
            of the policies. The categories are 'Low', 'Medium', 'High', and 'Very High'. The y-axis shows the average ROI for each category. 
            **Key Takeaways:** - Higher categories, such as 'High' and 'Very High', indicate policies with better returns on investment. 
...
def visualize_policy_comparison(top_policies):
    if not top_policies.empty:
        ...
        # Simple Explanation
        st.write("""**What this graph shows:** This bar chart compares the top 3 policies based on their Expected ROI, Investment Horizon, 
            and Potential Return. Each policy is represented by three bars: one for ROI, one for Horizon, and one for Potential Return. 
            **Key Takeaways:** - The higher the ROI, the better the potential return on your investment. 
